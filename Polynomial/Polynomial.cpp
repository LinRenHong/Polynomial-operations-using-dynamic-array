...
Polynomial Polynomial::operator + ( const Polynomial& poly )
{
    ...
    if( poly.length < length ) //判斷最長和最短
    {
        L = length;  //L為最長的長度
        S = poly.length; //S為最短的長度
    ...
    for( i = 0; i < S; i++ ) //迴圈從常數項開始一直到小於最短長度S
        result.terms[ i ].coef = poly.terms[ i ].coef + terms[ i ].coef; //次方一樣的相加
    
    for( i = L - 1; i >= S; i-- ) //迴圈從最長度L開始到最短長度S
        result.terms[ i ].coef = big.terms[ i ].coef; //把剩下沒加的的補回去
...
Polynomial Polynomial::operator * ( const Polynomial& poly )
{
    ...
    //建立相乘後的多項式
    Polynomial result( longestlength );
    ...
    for( int i = 0; i < length; i++ )
    {
        ...
        {
            //係數相乘
            result.terms[ i + j ].coef += terms[ i ].coef * poly.terms[ j ].coef;
            //指數相加
            result.terms[ i + j ].exp = terms[ i ].exp + poly.terms[ j ].exp;
        }
...
double Polynomial::solution( const int &x )
{
    ...
    //將值帶入
    for(int i = 0 ; i < length ;i++)
        total += terms[ i ].coef * pow( x , terms[ i ].exp);
